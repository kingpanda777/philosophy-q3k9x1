...
let bad = 0;
const run = ([kind, cmd]) => spawnSync(process.execPath, [G], { input: JSON.stringify({ tool_name: kind, tool_input: { command: cmd } }), encoding: 'utf8' });
...
console.log('止めるべき ' + BLOCK.length + '件・通すべき ' + (PASS.length + 2) + '件：' + (bad ? '食い違い ' + bad + '件' : 'すべて期待どおり'));
